...
if ($user) 
{
    ...
    if (isset($_POST['action']))
    {
        ...
        else if ($action == 'del_acc_list')
        {
            ...
            {
                $sql_check_ID_USER_exist = "SELECT ID_USER FROM user_account WHERE ID_USER = '$ID_USER'";
                if ($db->num_rows($sql_check_ID_USER_exist))
                {
                    $sql_del_acc = "DELETE FROM user_acount WHERE ID_USER = '$ID_USER'";
    ...
    // Xoá 1 tài khoản
        ...
        {       
            ...
            if ($db->num_rows($sql_check_ID_USER_exist))
            {
                $sql_del_acc = "DELETE FROM user_account WHERE ID_USER = '$ID_USER'";
                $db->query($sql_del_acc);
                $db->close();
            }       
        }
